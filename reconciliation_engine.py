...
def parse_tally(df):
    ...
    for col in ["IGST","CGST","SGST","Invoice_Value"]:
    ...
    return df, no_itc_df, pd.DataFrame()
...
def parse_gstr2b(df):
    ...
    for col in required:
        if col not in df.columns:
            raise Exception("Incorrect GSTR-2B format.")
...
def reconcile(gstr2b_df, tally_df):
    ...
    merged["VALUE_MATCH"] = (merged["Taxable_Value_2B"] - merged["Taxable_Value_Tally"]).abs() <= 1
    merged["TAX_MATCH"] = (merged["TOTAL_TAX_2B"] - merged["TOTAL_TAX_Tally"]).abs() <= 1
    ...
    summary = {
        ...
        "Match_Percentage": round((len(fully_matched)/len(gstr2b_df)*100) if len(gstr2b_df)>0 else 0,2),
        "Total_ITC_Books": round(tally_df["TOTAL_TAX"].sum(),2),
        "Total_ITC_2B": round(gstr2b_df["TOTAL_TAX"].sum(),2),
        "ITC_Difference": round(gstr22b_df["TOTAL_TAX"].sum()-tally_df["TOTAL_TAX"].sum(),2)
    }
